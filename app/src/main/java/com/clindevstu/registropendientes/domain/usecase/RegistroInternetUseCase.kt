...
class RegistroInternetUseCase @Inject constructor(
    ...
    suspend fun obtenerPaginados(tecnico: String): Result<PaginationResponse<RegistroInternetSimpleResponse>> {
        return try {
    ...
    suspend fun obtenerPorId(id: String): Result<RegistroInternetCompletoResponse?>{
        return try {
    ...
    suspend fun crearNuevoRegistro(registroRequest: RegistroInternetRequest): Result<RegistroResponse?>{
        return try {
