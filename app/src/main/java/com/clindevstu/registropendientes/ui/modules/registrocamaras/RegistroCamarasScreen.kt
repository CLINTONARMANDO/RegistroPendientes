...
fun RegistroCamarasScreen (navController: NavHostController, viewModel: RegistroCamarasViewModel){
    ...
    Scaffold(
        topBar = {
            CTopBarSecondary(
                title = "Registro Camaras",
                subtitle = "Registro de Clientes Camaras",
        ...
        Column(
            ...
            when (state) {
                is RegistroCamarasState.Init,
                is RegistroCamarasState.Loading -> {
                    // Puedes poner un shimmer o CircularProgressIndicator
                    CircularProgressIndicator(modifier = Modifier.padding(16.dp))
                }

                is RegistroCamarasState.Error -> {
                    val message = (state as RegistroCamarasState.Error).message
                    Text(
                        text = "Error: $message",
                ...
                is RegistroCamarasState.Success -> {
                    ...
                    if (listaRegistros.isEmpty()) {
                        ...
                        listaRegistros.forEach { registro ->
                            ...
                            CCardGridData(
                                principalIcon = painterResource(id = R.drawable.registro_camaras_icon),
                                title = registro.id ?: "Sin Cliente",
                                subTitle = registro.para_el_dia ?: "Sin Dirección",
                                thumbnailIcon = painterResource(R.drawable.menu_24px),
                                estado = registro.estado ?: "Desconocido",
                                headers = listOf("Asesor", "Empresa", "Ciudad", "Tipo", "Adelando", "FechaRegistro"),
                                data = listOf(
                                    registro.asesor ?: "N/A",
                                    registro.empresa ?: "N/A",
                                    registro.ciudad_zona ?: "N/A",
                                    registro.tipo_registro ?: "N/A",
                                    registro.adelanto ?: "N/A",
                                    AppFunctions.formatearFecha(registro.fecha_registro),
                                ),
                                columns = 3,
    ...
    if (isDetalleDialogActive) {
        CFullScreenModal(
            ...
            RegistroCamarasDetalleScreen(viewModel) // Aquí el detalle real
        }
    }
}
