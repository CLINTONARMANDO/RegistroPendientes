...
fun RegistroInternetScreen (navController: NavHostController, viewModel: RegistroInternetViewModel){
    ...
    Scaffold(
        ...
        Column(
            ...
            when (state) {
                is RegistroInternetState.Init,
                is RegistroInternetState.Loading -> {
                    // Puedes poner un shimmer o CircularProgressIndicator
                    CircularProgressIndicator(modifier = Modifier.padding(16.dp))
                }

                is RegistroInternetState.Error -> {
                    val message = (state as RegistroInternetState.Error).message
                    Text(
                        text = "Error: $message",
                ...
                is RegistroInternetState.Success -> {
                    ...
                    if (listaRegistros.isEmpty()) {
                        ...
                        listaRegistros.forEach { registro ->
                            ...
                            CCardGridData(
                                principalIcon = painterResource(id = R.drawable.registro_internet_icon),
                                title = registro.codigoGrande ?: "Sin Cliente",
                                subTitle = registro.diaInstalacion ?: "Sin Dirección",
